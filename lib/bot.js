...
function getBotReplyKey(message) {
  var key = message.channel + message.text.split("\n")[0];
  return key;
}

function checkBotReplies(theRequest) {
  var key = getBotReplyKey(theRequest);
  if (key in botReplies) {
    // already seen this reply
    var epoch = Math.floor((new Date).getTime()/1000);
    if (epoch - botReplies[key].ts > 30) {
      // but it was over 30 seconds ago
      return true;
    } else {
      return false;
    }
...
Bot.prototype.run = function () {
  ...
  _.each(self.config.jira_urls, function (value, key, obj) {
    ...
    if (value.jira) {
      var password = value.jira.password;
      delete value.jira.password;
  ...
  bot.use(function (message, cb) {
    if (message.subtype == "bot_message") {
      ...
      var key = getBotReplyKey(message);
      botReplies[key] = message;
    } else if ('message' == message.type && message.text != null && message.subtype != "bot_message" && typeof(message.reply_to) == 'undefined') {
      var requests = [];
            ...
            if (jiraApi && jiraApi != "NONE") {
              (function (def, jira) {
                ...
                jiraApi.findIssue(jira, function (error, issue) {
                  ...
                  var link = '';
                  if (issue) link = generateMsg(configUrl, issue, self.config.custom_texts.messagePrefix)
                  var req = {"channel": message.channel, "text": msg, "type": "issue", "link": link};
                  def.resolve(req);
                });
              })(q.defer(), j);
            } else {
          ...
          q.all(requests).then(function (messages) {
            _.forEach(messages, function (theRequest) {
              if (checkBotReplies(theRequest)) {
                ...
                //self.slacker.send('chat.update', message, {
                //  ts: message.ts,
                //  channel: message.channel,
                //  parse: "all",
                //  text: message.text + theRequest.text
                //});
              } else {
                if (verbose) {
                  console.log("Already seen the jira bot reply for this issue");
                  console.log("BotReplies:");
                  console.log(botReplies);
                }
              }
